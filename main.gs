function remind() {
  ...
  let switchstatus = switchsheet.getRange(1,2,1,1).getValue();
  ...
  for (let i = 2; i <= Math.min(upNextSheet.getLastRow(), 15); i++){
    let values = upNextSheet.getRange(i,1,1,4).getValues()[0];
