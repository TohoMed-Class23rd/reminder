function remind() {
  ...
  const switchsheet = sheets.getSheetByName("Switch");
  ...
  upNextSheet.getRange(2,1,Math.max(upNextSheet.getLastRow()-1,1),2).setNumberFormat("yyyy/MM/dd HH:mm");
  upNextSheet.getRange(2,1,Math.max(upNextSheet.getLastRow()-1,1),upNextSheet.getLastColumn()).sort({column: 2, ascending: true});
  let now = new Date();
  var content = '';

  if (switchstatus) {
    for (let i = 2; i <= Math.min(upNextSheet.getLastRow(), 11); i++){
      let values = upNextSheet.getRange(i,1,1,4).getValues().flat();
      let date = new Date(values[1]);
      let start_date = new Date(values[0]);
      if (now < date){
        // 未来
